...
model_save = "./modelSave/" + model_size[model_select - 1] + "_shangHaiAndWater_" + str(mBatchSize) + "_" + str(mLearningRate) + "/"

mkdir(model_save)

# per_label_nums_per_img = args.per_label_nums_per_img
per_label_nums_per_img = 60
...
if __name__ == '__main__':
    ...
    # 直接 输入 128 通道原始数据 吧 因为 网络第一层就是BN层！！！ 还是归一化把，本来就是根据趋势来选取！
    ...
    save_trainData_npy_path = save_npy_path + bands_str + str(intervalSelect) + "_" + str(per_label_nums_per_img) + '_31class.npy'
    mkdir(save_npy_path)
    ...
    if not os.path.exists(save_trainData_npy_path):
        ...
        # b c h w
        trainData, trainLabel = multiProcessGenerateData(dataType, per_label_nums_per_img, 11, train_bands,
                                                         'sig', nora= False, class_nums=class_nums,
    ...
    class_nums_jm = 31
    # 0: water, 30:other，其他类别见高光谱数据集标注详情！
    for i in range(class_nums_jm):
